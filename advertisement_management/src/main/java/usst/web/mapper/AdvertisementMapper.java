...
public interface AdvertisementMapper {
    ...
    @Insert("INSERT INTO advertisement(ad_id, ad_name, ad_url, ad_image_url, ad_start_time, ad_end_time, ad_use, ad_feature) VALUES(#{adId}, #{adName}, #{adUrl}, #{adImageUrl}, #{adStartTime}, #{adEndTime}, #{adUse}, #{adFeature})")
    @Options(useGeneratedKeys = true, keyProperty = "adId")
    void insertAdv(Advertisement advertisement);

    @Update("UPDATE advertisement SET ad_name=#{adName}, ad_url=#{adUrl}, ad_image_url=#{adImageUrl}, ad_start_time=#{adStartTime}, ad_end_time=#{adEndTime}, ad_use=#{adUse}, ad_feature=#{adFeature} WHERE ad_id=#{adId}")
